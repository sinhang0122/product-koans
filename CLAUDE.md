...
- **고아 이미지 정리:** 루트 `cleanup-orphan-images.js` (Node admin SDK) — **분기마다 1회 dry-run** 권장. 기본 dry-run(카운트+목록+총용량), `--delete` 로만 실삭제. 가드 내장: `post_images/used,maintenance/`(localStorage 보드 — Firestore 참조 없어 고아 오판) + 24h 이내 파일 제외. 서비스 계정 키는 **로컬만**(`.gitignore` 처리 — repo 커밋 금지).
