...
- **마퀴(공지 띠배너):** **운영자 한 줄 알림 전용** (2026-06 확정). `marquee.js` 는 `notices.text` 만 노출하고 좌측 배지는 **`📢 KoAus` 고정** — **광고주/게시자명(sponsor) 렌더 분기를 다시 만들지 말 것**(과거 2회 회귀: `t.sponsor` 폴백 노출). 광고주 강조는 히어로 배너+타겟 광고로 일원화. admin.html 공지 폼은 sponsor 입력 없음(text+우선순위+링크만). `notices.sponsor` 필드는 rules `optStr` 로 남겨 옛 데이터 호환하되 **신규 저장 안 함**.
