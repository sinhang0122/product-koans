...
- **게시글 스키마 필드 추가 시:** 클라이언트(`accom/rent/jobs/auto.html` 등) payload 에 새 키를 넣으려면 `firestore.rules` 의 `validPostCreate()` 안 `data().keys().hasOnly([...])` 화이트리스트도 함께 갱신하고 필요시 `optStr/optInt/optBool/optUrlList` 검증도 추가하라. 누락 시 `create` 가 `permission-denied` 로 거부되어 글쓰기가 즉시 멈춘다. 배포 순서는 **룰 먼저(`npm run deploy:rules`) → 새 클라이언트 나중(`firebase deploy --only hosting`)**. 순서를 거꾸로 하면 옛 룰이 새 키를 차단해 출시 직후 첫 글쓰기가 깨진다.
